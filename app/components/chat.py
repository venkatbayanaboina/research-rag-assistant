...
def render_chat_interface(use_rerank, indexed_docs):
    ...
    if st.session_state.thinking and st.session_state.current_prompt:
        ...
        try:
            ...
            routing = route_user_intent(active_prompt, indexed_docs)
            ...
            if intent == "COMPARISON" and len(targets) >= 2:
                ...
                with st.spinner("Searching text database..."):
                    search_results = search_store(active_prompt, rerank=use_rerank)
                    if targets:
                ...
                with st.spinner("Searching visual database with CLIP..."):
                    image_results = search_image_store(active_prompt)
                    if targets:
                ...
                with st.spinner("Gemini is thinking..."):
                    try:
                        answer = generate_answer(active_prompt, search_results, image_results, st.session_state.chat_history[:-1])
                    except Exception as e:
